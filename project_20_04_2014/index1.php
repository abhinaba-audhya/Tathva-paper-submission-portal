...
if (isset($_POST['formsubmitted'])) {
    ...
    else
    {
		...
		{//if no name has been supplied 
            $error[] = 'Please Enter a phone-no ';//add to array "error"
        } 
		else 
		{
		...
		{//if no name has been supplied 
            $error[] = 'Please Enter a College name ';//add to array "error"
        } 
		else
		{
            $college = $_POST['college'];//else assign it a variable
        }
        if (empty($_POST['roll'])) 
		{//if no name has been supplied 
            $error[] = 'Please Enter a roll-no ';//add to array "error"
        }
		else
		{
            $roll = $_POST['roll'];//else assign it a variable
            $result_verify_email = mysqli_query($dbc, "SELECT * FROM User WHERE Roll='$roll' and Institute='$college'");
            if (!$result_verify_email) {//if the Query Failed ,similar to if($result_verify_email==false)
                $error[]=' Database Error Occured ';}
            else if (mysqli_num_rows($result_verify_email) > 0)
                $error[]='Roll already enrolled';
    ...
    { // If everything's OK...
        ...
        $result_verify_email = mysqli_query($dbc, "SELECT * FROM User WHERE Email ='$Email'");
        if (!$result_verify_email) {//if the Query Failed ,similar to if($result_verify_email==false)
        ...
        if (mysqli_num_rows($result_verify_email) == 0) { // IF no previous user is using this email .
            ...
            $query_insert_user = "INSERT INTO User ( Username, Email, Password, Activation, Institute , Roll, Phone ) VALUES ( '$name', '$Email', '$Password', '$activation' , '$college','$roll','$phone')";
    	...
    	//echo $message;
				...
				mail($Email, 'Registration Confirmation', $message, 'From: server_norply@allstuffcodes.info');
...
<form action="index1.php" method="post" class="registration_form">
  <fieldset>
    ...
    <div class="elements">
      ...
      <input type="text" id="roll" name="roll" size="25" />
    </div>
